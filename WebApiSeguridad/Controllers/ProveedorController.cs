...
namespace WebApiSeguridad.Controllers
{
    ...
    public class ProveedorController : ControllerBase
    {
        ...
        private Service.ProveedorSER objBss;

        public ProveedorController(IConfiguration configuration, IMapper mapper)
        {
        ...
        [HttpGet("ListarAllProveedores")]
        public ActionResult<List<DTO.ProveedorListarDTO>> ListarAllProveedores()
        {
            try
            {
        ...
        [HttpGet("FiltrarProveedores")]
        public ActionResult<List<DTO.ProveedorListarDTO>> FiltrarProveedores([FromQuery] string inicio, [FromQuery] string fin, [FromQuery] string nombre = "")
        {
            try
            {
        ...
        public ActionResult RegistrarProveedor([FromBody] ProveedorAgregarDTO dto)
        {
            try
            {
                ...
                if (resultado > 0)
                {
                    return Ok(new { message = "Proveedor creado exitosamente", id = resultado });
                }
                else
                {
        ...
        public ActionResult ModificarProveedor([FromBody] ProveedorModificarDTO dto)
        {
            try
            {
                if (!ModelState.IsValid)
                ...
                {
                    return BadRequest(new { message = "No se pudo modificar el proveedor" });
                }
            }
            catch (Exception ex)
            {
        ...
        public ActionResult EliminarProveedor(int id)
        {
            try
            {
                ...
                if (resultado > 0)
                {
                    return Ok(new { message = "Proveedor deshabilitado exitosamente" });
                }
                else
                {
                    return BadRequest(new { message = "No se pudo deshabilitar el proveedor" });
                }
            }
            catch (Exception ex)
            {
